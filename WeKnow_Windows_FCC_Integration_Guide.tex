...
%  Auth & Capture  --  Version 1.0 (July 2026)
%
...
\fancyhead[R]{\raisebox{-0.35\height}{\includegraphics[height=15pt]{wkg-logo-horizontal}}}
...
\titleformat{\subsubsection}
  {\color{weknowgraphite}\large\bfseries}
  {\color{weknowazure}\thesubsubsection}{0.6em}{}
...
%=====================================================================
% COVER  (We Know Group brand: white-dominant, logo top-left @ 52mm/10mm
%         per print spec, Azure as accent -- signature stripe, wink-echo
%         rounded-square motif in Azure tints, WeKnow voice headline)
%=====================================================================
\begin{titlepage}
\thispagestyle{empty}
\begin{tikzpicture}[remember picture,overlay]
  % --- Left Azure accent stripe (brand signature spine) ---
  \fill[weknowazure] (current page.north west) rectangle
       ([xshift=6mm]current page.south west);

  % --- 'Wink' echo: nested Azure-tint rounded squares, top-right (bleed) ---
  \fill[weknowazure20,rounded corners=10mm]
       ([xshift=-58mm,yshift=-52mm]current page.north east) rectangle
       ([xshift=16mm,yshift=16mm]current page.north east);
  \fill[weknowazure40,rounded corners=7mm]
       ([xshift=-40mm,yshift=-40mm]current page.north east) rectangle
       ([xshift=12mm,yshift=10mm]current page.north east);
  \fill[weknowazure,rounded corners=5mm]
       ([xshift=-28mm,yshift=-34mm]current page.north east) rectangle
       ([xshift=6mm,yshift=4mm]current page.north east);

  % --- Official WeKnow logo: top-left, 52mm @ 10mm margin (brand spec) ---
  \node[anchor=north west]
    at ([xshift=16mm,yshift=-14mm]current page.north west)
    {\includegraphics[width=52mm]{wkg-logo-horizontal}};

  % --- Headline (WeKnow voice: wordmark + lowercase follow-on) ---
  \node[anchor=west,align=left]
    at ([xshift=16mm,yshift=34mm]current page.west)
    {\color{weknowgreyN}\fontsize{22}{26}\selectfont how to connect};
  \node[anchor=west,align=left,font=\bfseries]
    at ([xshift=16mm,yshift=4mm]current page.west)
    {\color{weknowgraphite}\fontsize{40}{46}\selectfont FreedomPay\\[2pt]Windows FCC};
  % Azure hero rule (highlights the key element)
  \fill[weknowazure]
    ([xshift=17mm,yshift=-18.5mm]current page.west) rectangle
    ([xshift=61mm,yshift=-16mm]current page.west);
  \node[anchor=west,align=left]
    at ([xshift=16mm,yshift=-28mm]current page.west)
    {\color{weknowsea}\fontsize{18}{22}\selectfont Auth \& Capture Integration Guide};

  % --- T&Cs: bottom-left, range left (brand supporting-element spec) ---
  \node[anchor=south west,align=left]
    at ([xshift=16mm,yshift=18mm]current page.south west)
    {{\color{weknowgraphite}\normalsize\bfseries Version 1.0}\quad
     {\color{weknowgreyN}\normalsize July 2026}\\[5pt]
     {\color{weknowgreyN}\footnotesize We Know Group --- Internal Implementation \& Support Documentation}\\[2pt]
     {\color{weknowgreyN}\footnotesize Prepared for FreedomPay certification per Certification Documentation Requirements Guide v1.9}};

  % --- URL: bottom-right (brand supporting-element spec) ---
  \node[anchor=south east]
    at ([xshift=-16mm,yshift=18mm]current page.south east)
    {\color{weknowsea}\small www.WeKnowLondon.com};
\end{tikzpicture}
\end{titlepage}
...
1.0 & 2026-07-10 & WeKnow Integrations &
Initial release --- Windows FCC Auth \& Capture integration and
certification documentation for standalone deployments (FCC 5.4.x):
system overview and data-flow diagrams, setup and configuration
(including US, UK, EU and Australia regional settings), and the
...
\paragraph{4. Australia}
\begin{lstlisting}
<!-- FreewayServerService.exe.config -->
<add key="baseCulture" value="en-AU" />
<add key="receiptFormatter" value="global2" />
<add key="merchantCurrencyCode" value="AUD" />

<!-- FreewayClientService.exe.config -->
<add key="dcc.localCurrency" value="AUD" />
<add key="dcc.localCurrencyCode" value="036" />
\end{lstlisting}

Australia uses FreedomPay's \texttt{global2} receipt formatter (not a
country-specific value such as \texttt{us} or \texttt{uk}), and AUD carries
the ISO numeric currency code \texttt{036}. Where Dynamic Currency
Conversion is offered, the AU BIN-range list
(\texttt{DCC/GBCS.AUD.TXT}) and the DCC disclaimer keys also apply ---
see the FreedomPay localisation reference at
\url{https://docs.freedompay.com/fcc/configurations/localization/australia}.
...
\centering
\setlength{\fboxsep}{0pt}\setlength{\fboxrule}{0.6pt}%
{\color{weknowgreyM}\fbox{\includegraphics[width=0.30\linewidth]{weknow-txn-approved}}}\hfill
{\color{weknowgreyM}\fbox{\includegraphics[width=0.30\linewidth]{weknow-txn-error}}}\hfill
{\color{weknowgreyM}\fbox{\includegraphics[width=0.30\linewidth]{weknow-txn-decline}}}
